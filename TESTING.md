...
This document provides an overview of the test suite for the
targets.utils package.
...
The test suite contains **68 comprehensive tests** covering all public
functions in targets.utils:

### Functions Tested

1.  **[`tl()`](https://geryan.github.io/targets.utils/reference/tl.md)** -
    Load all globals and targets
    - 5 tests
...
3.  **[`insert_tar_target()`](https://geryan.github.io/targets.utils/reference/insert_tar_target.md)
    / `it()`** - RStudio addin
    - 10 tests
    - Tests function availability, signatures, and aliasing
4.  **[`tar_terra_nested()`](https://geryan.github.io/targets.utils/reference/tar_terra_nested.md)** -
    Handle nested spatial objects
...
- [x] Creates valid target objects
- [x] Handles lists with SpatRaster objects
- [x] Handles lists with SpatVector objects
- [x] Handles nested lists with both rasters and vectors
- [x] Preserves non-spatial data in nested structures

#### Advanced Features

- [x] Respects custom raster_filetype parameter
- [x] Works with tibbles containing spatial list-columns
- [x] Correctly handles deeply nested spatial objects (3+ levels)
- [x] Creates valid tar_target with correct configuration
- [x] Format is custom tar_format_terra_nested
...
- [x]
  [`tar_format_terra_nested()`](https://geryan.github.io/targets.utils/reference/tar_format_terra_nested.md)
  creates a valid format object
...
- Tests run in temporary directories using
...
- Uses testthat’s clear assertion functions (`expect_true()`,
  `expect_equal()`, etc.)
...
Tests will: - Skip RStudio-dependent tests in CI environments - Skip
tests for missing optional dependencies - Exit with appropriate code on
failure
...
1.  **Descriptive names**: Use `test_that("description", ...)` with
    clear descriptions
2.  **Setup-Execute-Assert**: Follow the 3A pattern
3.  **One assertion per test**: Prefer multiple focused tests over one
    test with many assertions
4.  **Skip unavailable dependencies**: Use `skip_if_not_installed()`
5.  **Isolate side effects**: Use
    [`tempfile()`](https://rdrr.io/r/base/tempfile.html) for file system
    operations
6.  **Clean up**: Ensure temporary files are deleted (e.g.,
...
Individual test files: - `test-tl.R`: ~0.5s - `test-tml.R`: ~1.5s -
`test-insert-tar-target.R`: ~0.3s - `test-tar-terra-nested.R`: ~1.1s
